<template>
    <div class="app">
        ...
        <div class="container" v-loading.body="loading">
            <div class="content">
                <div class="item" v-for="(e, index) in content_data" :key="index">
                    <a-card :body-style="getBodyStyle(e.id)">
                        <div class="card-des">
                            <div class="des-img">
        ...
        <div class="footer">
            <a-button type="primary" @click="next_step===0?next():complete()"> {{next_step === 0 ? '下一步' : '提交'}}
            </a-button>
        </div>
        <a-dialog title="神经网络预测结果" :visible.sync="resultDialogVisible">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.loading = true;
            axios.post('http://155.138.165.114:3001/ann/getNetwork')
                .then((response) => {
            ...
            // const mock = () => {
                ...
                for (let i = 0; i < 2000; i++) {
                    // 0,0 => 0
                    localNetworkInstance.activate([1,0,0,0,0]);
                    localNetworkInstance.propagate(learningRate, [1,1,1,1,0,0,0,1,0,1]);

                    localNetworkInstance.activate([0,1,0,0,0]);
                    localNetworkInstance.propagate(learningRate, [1,1,1,1,1,0,1,0,0,0]);

                    localNetworkInstance.activate([0,0,1,0,0]);
                    localNetworkInstance.propagate(learningRate, [1,1,1,1,1,1,0,1,0,0]);
                }

                // test the network
                console.log(localNetworkInstance.activate([0,0,0,1,0]));
                console.log(localNetworkInstance.activate([0,0,0,0,1])); // [0.015020775950893527]
            // }
        },
        methods: {
            shuffle(input) {
